...
<?php if ( $has_vouchers ) : ?>
	...
	<table class="woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		...
		<tbody>
			<?php
			foreach ( $customer_vouchers->vouchers as $voucher ) :
					...
					<?php foreach ( evwp_get_account_vouchers_columns() as $column_id => $column_name ) : ?>
						<td class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							elseif ( 'voucher-expiry' === $column_id ) :
								if ( $voucher->expiry > 0 ) :
									?>
								<time datetime="<?php echo esc_attr( date( 'Y-m-d', strtotime( $voucher->expiry ) ) ); ?>" title="<?php echo esc_attr( strtotime( $voucher->expiry ) ); ?>"><?php echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $voucher->expiry ) ) ); ?></time>
									<?php
								else :
									?>
	...
	<?php if ( 1 < $customer_vouchers->max_num_pages ) : ?>
		<div class="woocommerce-Pagination">
			...
			<?php if ( $current_page !== intval( $customer_vouchers->max_num_pages ) ) : ?>
